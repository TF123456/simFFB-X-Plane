...
static unsigned __stdcall ListenerThread(void *)
{
    ...
    while (g_running)
    {
        ...
        for (int i = 0; i < nvals; i++)
        {
            ...
            {
            case 0:
                g_tele.elevator_trim = vals[i].value;
                break;
            case 1:
                g_tele.aileron_trim = vals[i].value;
                break;
            case 2:
                g_tele.indicated_airspeed = vals[i].value;
                break;
            case 3:
                g_tele.stall_warning = vals[i].value;
                break;
            case 4:
                g_tele.g_normal = vals[i].value;
                break;
            case 5:
                g_tele.on_ground = vals[i].value;
                break;
            case 6:
                g_tele.elv_deflection = vals[i].value;
                break;
            case 7:
                g_tele.ail_deflection = vals[i].value;
                break;
            case 8:
                g_tele.tire_vrt_def[0] = vals[i].value;
                break;
            case 9:
                g_tele.tire_vrt_def[1] = vals[i].value;
                break;
            case 10:
                g_tele.tire_vrt_def[2] = vals[i].value;
                break;
            case 11:
...
{
    if (!g_running && g_sock == INVALID_SOCKET)
        return;
