...
HWND g_hwnd;
const int CLIENT_WIDTH = 540; // desired client area dimensions
const int CLIENT_HEIGHT = 415;
HMENU g_hm;
...
BOOL is_iKeyDown = false;
BOOL is_ctKeyDown = false;
...
// X-Plane controls
HWND hwndCHXPlane;       // X-Plane mode checkbox
HWND hwndEBXPlaneIP;     // IP address edit box
HWND hwndEBXPlanePort;   // Port edit box
HWND hwndLBXPlaneStatus; // Connection status label
HWND hwndTBShakerGain;   // Shaker gain trackbar
...
HWND hwndLBAilDiag;                   // live aileron force readout
HWND hwndLBTrimY;                     // trim Y value readout
HWND hwndLBTrimX;                     // trim X value readout
HWND hwndBtnResetTrim;                // reset trim to zero
...
BOOL *GetModifierConditions(BOOL *modArr);
    ...
    while (true)
    {
        ...
        { // no events, do you stuff here
            ...
            else
            {
                if (jopt.xplaneMode)
                {
                    // POV hat adjusts internal trim offset (g_nYForce/g_nXForce);
                    // ApplyXPlaneTelemetry combines it with the aero load in SetDeviceSpring.
                    if (g_ptrim)
                        JoystickStuffPT();
                    ApplyArrowKeyTrim();
                    XPlaneTelemetry tele;
                    GetTelemetry(&tele);
                    // Update status label only when state changes to avoid redraws
                    if (tele.connected != g_xplaneStatusConnected)
                    {
                        SetWindowText(hwndLBXPlaneStatus, tele.connected ? _T("Connected") : _T("Disconnected"));
                        g_xplaneStatusConnected = tele.connected;
                    }
                    if (tele.connected)
                        ApplyXPlaneTelemetry(tele);
                    // Live force diagnostics — always update when X-Plane mode active
                    {
                        INT aeroY, aeroX, trimY, trimX, effTrimY, effTrimX;
                        GetXPlaneForces(aeroY, aeroX, trimY, trimX, effTrimY, effTrimX);
                        TCHAR buf[256];
                        _stprintf_s(buf, 256, _T("def=%+5.1f"), tele.elv_deflection);
                        SetWindowText(hwndLBElvDiag, buf);
                        _stprintf_s(buf, 256, _T("def=%+5.1f"), tele.ail_deflection);
                        SetWindowText(hwndLBAilDiag, buf);
                        _stprintf_s(buf, 256, _T("%+4d"), trimY * 100 / DI_FFNOMINALMAX);
                        SetWindowText(hwndLBTrimY, buf);
                        _stprintf_s(buf, 256, _T("%+4d"), trimX * 100 / DI_FFNOMINALMAX);
                        SetWindowText(hwndLBTrimX, buf);
                    }
                }
                else
                {
                    ...
                    if (g_ptrim)
                        JoystickStuffPT();
                    ApplyArrowKeyTrim();
                    // Update trim display in manual mode too
                    {
                        TCHAR buf[16];
                        INT aeroY, aeroX, trimY, trimX, effTrimY, effTrimX;
                        GetXPlaneForces(aeroY, aeroX, trimY, trimX, effTrimY, effTrimX);
                        _stprintf_s(buf, 16, _T("%+4d"), trimY * 100 / DI_FFNOMINALMAX);
                        SetWindowText(hwndLBTrimY, buf);
                        _stprintf_s(buf, 16, _T("%+4d"), trimX * 100 / DI_FFNOMINALMAX);
                        SetWindowText(hwndLBTrimX, buf);
                    }
                }
                ...
                if ((1 << 16) & GetAsyncKeyState(jopt.iKey) && !is_iKeyDown)
                {
                    BOOL *modifierConditions = GetModifierConditions(jopt.iKeyMod);
                    if (modifierConditions[0] && modifierConditions[1] &&
                        modifierConditions[2] && modifierConditions[3])
                    {
                        InitAll(false);
                    }
                    is_iKeyDown = true;
                }
                else if (GetAsyncKeyState(jopt.iKey) == 0)
                {
                    is_iKeyDown = false;
                }

                if ((1 << 16) & GetAsyncKeyState(jopt.ctKey) && !is_ctKeyDown)
                {
                    BOOL *modifierConditions = GetModifierConditions(jopt.ctKeyMod);
                    if (modifierConditions[0] && modifierConditions[1] &&
                        modifierConditions[2] && modifierConditions[3])
                    {
                ...
                else if (GetAsyncKeyState(jopt.ctKey) == 0)
                {
                    is_ctKeyDown = false;
                }
            }
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    LoadOptionsFromFile();
    GetJtOptions(&jopt);
    // Compute exact window size from desired client area (accounts for title bar, borders, menu bar)
    RECT rc = {0, 0, CLIENT_WIDTH, CLIENT_HEIGHT};
    AdjustWindowRect(&rc, WS_SYSMENU | WS_CAPTION | WS_MINIMIZEBOX | WS_DLGFRAME, TRUE);
    ...
    // Clamp saved position so the window never opens off-screen
    {
        ...
        if (jopt.windowX < 0 || jopt.windowX + winW > sw)
            jopt.windowX = 0;
        if (jopt.windowY < 0 || jopt.windowY + winH > sh)
            jopt.windowY = 0;
    }
    hWnd = CreateWindow(szWindowClass, szTitle, WS_SYSMENU | WS_CAPTION | WS_MINIMIZEBOX | WS_DLGFRAME, jopt.windowX, jopt.windowY, winW, winH, NULL, NULL, hInstance, NULL);
    ...
    CreateWindow(_T("static"), _T("Hold"), WS_CHILD | WS_VISIBLE, 370, 3, 130, 20, hWnd, NULL, hInstance, NULL);
    CreateWindow(_T("static"), _T("Toggle"), WS_CHILD | WS_VISIBLE, 420, 3, 130, 20, hWnd, NULL, hInstance, NULL);
    CreateWindow(_T("static"), _T("Center"), WS_CHILD | WS_VISIBLE, 470, 3, 130, 20, hWnd, NULL, hInstance, NULL);
    hwndCBSticks = CreateWindow(_T("combobox"), TEXT(""), CBS_DROPDOWNLIST | WS_CHILD | WS_VISIBLE, 3, 23, 360, 255, hWnd, NULL, hInstance, NULL);
    hwndCBTrimHold = CreateWindow(_T("combobox"), TEXT(""), CBS_DROPDOWNLIST | WS_VSCROLL | WS_CHILD | WS_VISIBLE, 370, 23, 50, 255, hWnd, NULL, hInstance, NULL);
    hwndCBTrimToggle = CreateWindow(_T("combobox"), TEXT(""), CBS_DROPDOWNLIST | WS_VSCROLL | WS_CHILD | WS_VISIBLE, 420, 23, 50, 255, hWnd, NULL, hInstance, NULL);
    hwndCBTrimCenter = CreateWindow(_T("combobox"), TEXT(""), CBS_DROPDOWNLIST | WS_VSCROLL | WS_CHILD | WS_VISIBLE, 470, 23, 50, 255, hWnd, NULL, hInstance, NULL);
    ...
    hwndCBSticksPOV = CreateWindow(_T("combobox"), TEXT(""), CBS_DROPDOWNLIST | WS_CHILD | WS_VISIBLE, 3, 53, 360, 255, hWnd, NULL, hInstance, NULL);

    CreateWindow(_T("static"), _T("SimFFB"), WS_CHILD | WS_VISIBLE | WS_BORDER | ES_CENTER, 370, 55, 149, 20, hWnd, NULL, hInstance, NULL);
    ...
    hwndTBSpring = CreateWindow(TRACKBAR_CLASS, NULL, WS_CHILD | WS_VISIBLE | TBS_ENABLESELRANGE, 168, 83, 356, 20, hWnd, NULL, hInstance, NULL);
    ...
    hwndTBDamper = CreateWindow(TRACKBAR_CLASS, NULL, WS_CHILD | WS_VISIBLE | TBS_ENABLESELRANGE, 168, 103, 356, 20, hWnd, NULL, hInstance, NULL);
    ...
    hwndTBFriction = CreateWindow(TRACKBAR_CLASS, NULL, WS_CHILD | WS_VISIBLE | TBS_ENABLESELRANGE, 168, 123, 356, 20, hWnd, NULL, hInstance, NULL);
    ...
    hwndTBSpring2 = CreateWindow(TRACKBAR_CLASS, NULL, WS_CHILD | WS_VISIBLE | TBS_ENABLESELRANGE, 168, 143, 356, 20, hWnd, NULL, hInstance, NULL);
    ...
    hwndTBDamper2 = CreateWindow(TRACKBAR_CLASS, NULL, WS_CHILD | WS_VISIBLE | TBS_ENABLESELRANGE, 168, 163, 356, 20, hWnd, NULL, hInstance, NULL);
    ...
    hwndTBFriction2 = CreateWindow(TRACKBAR_CLASS, NULL, WS_CHILD | WS_VISIBLE | TBS_ENABLESELRANGE, 168, 183, 356, 20, hWnd, NULL, hInstance, NULL);
    ...
    // Init dinput hotkey
    CreateWindow(_T("static"), _T("Init Key"), WS_CHILD | WS_VISIBLE | ES_CENTER, 90, 239, 80, 20, hWnd, NULL, hInstance, NULL);
    hwndCBInitDInput = CreateWindow(_T("combobox"), _T(""), CBS_DROPDOWNLIST | WS_VSCROLL | WS_CHILD | WS_VISIBLE | ES_CENTER, 160, 235, 140, 300, hWnd, NULL, hInstance, NULL);
    // Cycle trim hotkey
    CreateWindow(_T("static"), _T("Trim Mode"), WS_CHILD | WS_VISIBLE | ES_CENTER, 300, 239, 80, 20, hWnd, NULL, hInstance, NULL);
    hwndCBCycleTrim = CreateWindow(_T("combobox"), _T(""), CBS_DROPDOWNLIST | WS_VSCROLL | WS_CHILD | WS_VISIBLE | ES_CENTER, 380, 235, 140, 300, hWnd, NULL, hInstance, NULL);

    // X-Plane FFB row 1: checkbox, IP, port, status
    hwndCHXPlane = CreateWindow(_T("button"), _T("X-Plane FFB"), WS_CHILD | WS_VISIBLE | BS_CHECKBOX, 3, 272, 100, 20, hWnd, NULL, hInstance, NULL);
    CreateWindow(_T("static"), _T("IP"), WS_CHILD | WS_VISIBLE, 108, 272, 15, 20, hWnd, NULL, hInstance, NULL);
    hwndEBXPlaneIP = CreateWindow(_T("Edit"), _T("127.0.0.1"), WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT, 125, 272, 110, 20, hWnd, NULL, hInstance, NULL);
    CreateWindow(_T("static"), _T(":"), WS_CHILD | WS_VISIBLE, 238, 272, 8, 20, hWnd, NULL, hInstance, NULL);
    ...
    hwndLBElvDiag = CreateWindow(_T("static"), _T("---"), WS_CHILD | WS_VISIBLE, 40, 330, 495, 18, hWnd, NULL, hInstance, NULL);
    CreateWindow(_T("static"), _T("Ail:"), WS_CHILD | WS_VISIBLE, 3, 353, 35, 18, hWnd, NULL, hInstance, NULL);
    hwndLBAilDiag = CreateWindow(_T("static"), _T("---"), WS_CHILD | WS_VISIBLE, 40, 353, 495, 18, hWnd, NULL, hInstance, NULL);

    // Trim display + reset
    CreateWindow(_T("static"), _T("TrimY:"), WS_CHILD | WS_VISIBLE, 3, 377, 42, 18, hWnd, NULL, hInstance, NULL);
    hwndLBTrimY = CreateWindow(_T("static"), _T("  0"), WS_CHILD | WS_VISIBLE, 47, 377, 55, 18, hWnd, NULL, hInstance, NULL);
    CreateWindow(_T("static"), _T("TrimX:"), WS_CHILD | WS_VISIBLE, 110, 377, 42, 18, hWnd, NULL, hInstance, NULL);
    hwndLBTrimX = CreateWindow(_T("static"), _T("  0"), WS_CHILD | WS_VISIBLE, 154, 377, 55, 18, hWnd, NULL, hInstance, NULL);
    hwndBtnResetTrim = CreateWindow(_T("button"), _T("Reset Trim"), WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 220, 375, 90, 20, hWnd, NULL, hInstance, NULL);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case CBN_SELCHANGE:
            ...
            break;
        case BN_CLICKED:
            if ((HWND)lParam == hwndCHXPlane)
            {
                jopt.xplaneMode ^= 1;
                SendMessage(hwndCHXPlane, BM_SETCHECK,
                            jopt.xplaneMode ? BST_CHECKED : BST_UNCHECKED, 0);
                if (jopt.xplaneMode)
                {
                    char ip[64];
                    GetWindowTextA(hwndEBXPlaneIP, ip, sizeof(ip));
                    TCHAR portBuf[16];
                    GetWindowText(hwndEBXPlanePort, portBuf, sizeof(portBuf) / sizeof(portBuf[0]));
                    int port = _ttoi(portBuf);
                    if (port <= 0)
                        port = 49000;
                    StartXPlaneListener(ip, port);
                }
                else
                {
                    StopXPlaneListener();
                    g_xplaneSpeedScale = 1.0f;
                    SetShakerActive(false, 0);
                    SetBumpActive(false, 0);
                    SetWindowText(hwndLBXPlaneStatus, _T("Disconnected"));
                    g_xplaneStatusConnected = false;
                }
                SetJtOptions(&jopt);
            }
            if ((HWND)lParam == hwndBtnResetTrim)
            {
                ResetTrim();
            }
    ...
    case WM_HSCROLL:
        ...
        else if ((HWND)lParam == hwndTBShakerGain)
        {
            jopt.shakerGain = (int)SendMessage(hwndTBShakerGain, TBM_GETPOS, 0, 0);
            TCHAR *tmp = new TCHAR[11];
            _stprintf_s(tmp, 11, _T("%i"), jopt.shakerGain);
            Edit_SetText(hwndEBShakerGain, tmp);
            delete[] tmp;
            SetJtOptions(&jopt);
        }
        else if ((HWND)lParam == hwndTBBumpGain)
        {
            jopt.bumpGain = (int)SendMessage(hwndTBBumpGain, TBM_GETPOS, 0, 0);
            TCHAR *tmp = new TCHAR[11];
            _stprintf_s(tmp, 11, _T("%i"), jopt.bumpGain);
...
void InitAll(BOOL firstrun)
{
    ...
    if (savedKeyIndexInit >= 0)
        ComboBox_SetCurSel(hwndCBInitDInput, savedKeyIndexInit);
    if (savedKeyIndexTrim >= 0)
        ComboBox_SetCurSel(hwndCBCycleTrim, savedKeyIndexTrim);
    ...
    {
        TCHAR ipW[64];
        MultiByteToWideChar(CP_ACP, 0, jopt.xplaneIP, -1, ipW, 64);
        SetWindowText(hwndEBXPlaneIP, ipW);

        TCHAR portBuf[16];
        _stprintf_s(portBuf, 16, _T("%i"), jopt.xplanePort > 0 ? jopt.xplanePort : 49000);
        SetWindowText(hwndEBXPlanePort, portBuf);
...
BOOL *GetModifierConditions(BOOL *modArr)
{
    BOOL modifierStates[4];
    BOOL modifierConditions[4];
    modifierStates[0] = GetAsyncKeyState(VK_CONTROL) & (1 << 16);
    modifierStates[1] = GetAsyncKeyState(VK_MENU) & (1 << 16);
    modifierStates[2] = GetAsyncKeyState(VK_SHIFT) & (1 << 16);
    modifierStates[3] = GetAsyncKeyState(VK_LWIN) & (1 << 16);

    for (int i = 0; i < 4; i++)
    {
        modifierConditions[i] = (modifierStates[i] && modArr[i]) ||
                                (!modifierStates[i] && !modArr[i]);
    }
    return modifierConditions;
}
